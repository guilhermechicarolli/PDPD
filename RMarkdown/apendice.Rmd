...
**Tabela 6:** Área potencial (em $km^{2}$) das espécies de acordo com o presente e os dois cenários climpaticos futuros. 
...
*Script* de código para a modelagem da distribuição potencial de *E. subsecundum*, disponível no repositório Git Hub do projeto em [https://github.com/guilhermechicarolli/PDPD/tree/main/Scripts_finais](https://github.com/guilhermechicarolli/PDPD/tree/main/Scripts_finais). O código para a modelagem da distribuição do morcego é totalmente análogo.

```{r, codigo, echo=TRUE, include = TRUE, message = FALSE, warning = FALSE, eval=FALSE, error=FALSE}
...
# Adicionamos uma coluna com a especie (necesaria como 
# argumento na modelagem)
spg$species <- 1
...
# Carregar as camadas e uma camada representante (bio 1) para corte 
# das camadas
cortef <- biof45
cam_repf <- biof45[[1]]
...
# Cortar as camadas ambientais e cortar a partir da mascara 
# criada do Brasil
...
# Carregar as camadas e uma camada representante (bio 1) para 
# corte das camadas
cortef85 <- biof85
cam_repf85 <- biof85[[1]]
...
# Adicionar os dados para fazer um modelo previo com todas as variaveis
dC <- sdm::sdmData(species~., spg, predictors = bioCams, 
                   bg=list(method='gRandom', n=10000))
dC

# Ajustar e criar os modelos
mC <- sdm::sdm(species~., dC, methods = 'maxent', 
               replication=c('sub', 'boot'),
               test.p=30, n=25, 
               parallelSettings=list(ncore=5, method='parallel'))

# NOTAS: 1) Para o MaxEnt funcionar o Java do computador deve estar 
# atualizado. 2) O parametro ncore refere-se a quantidade de cores de 
# processamento utilizados para a modelagem, altere conforme a capacidade
# do computador
...
plot(getVarImp(mC), 'AUC', main="Importância relativa das biovariáveis", 
     ylab='Variáveis', xlab="Importância relativa da variável")#Bio 6,17,19,14
...
d <- sdm::sdmData(species~., spg, predictors = bioc,
                  bg=list(method='gRandom', n=10000))
d

# Ajustar os modelos, 50 replicacoes, 25 por Subsampling e 25 por Bootstrap
m <- sdm::sdm(species~., d, methods='maxent', 
              replication=c('sub', 'boot'),
              test.p=30, n=25, 
              parallelSettings=list(ncore=5, method='parallel'))

# NOTAS: Como mencionado na secao 2, o parametro ncore refere-se a quantidade
# de cores de processamento utilizados para a modelagem, altere conforme a 
...
# Projecao dos 50 modelos criados na secao anterior para o presente
...
# Obter um modelo consenso dentre os 50 criados para o futuro RCP45 por meio 
# da mediana ponderada
en45 <- sdm::ensemble(m, bioS45, 
                      filename='./Resultados_subsecundum/futuro_RCP45.img',
                      setting =list(method='weighted', stat='tss', opt=2), 
                      overwrite=TRUE)
en45
...
# Obter um modelo consenso dentre os 50 criados para o futuro RCP85 por meio 
# da mediana ponderada
en85 <- sdm::ensemble(m, bioS85, 
                      filename='./Resultados_subsecundum/futuro_RC85.img',
                      setting =list(method='weighted', stat='tss', opt=2), 
                      overwrite=TRUE)
...
#----------
### Mapa de alteracao de adequabilidade (NAO BINARIO) entre o presente e o
# futuro de RCP 45
ch45 <- en45 - en
...
th <- ev@threshold_based$threshold[2] #limiar pelo metodo de max(espec+sens)
